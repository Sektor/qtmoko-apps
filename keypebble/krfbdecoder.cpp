...
void KRFBDecoder::gotDesktopName()
{
  ...
  //qWarning( "Desktop: %s", info->name.toLatin1() );
...
void KRFBDecoder::handleHexTileRect()
{
	...
	if ( currentState == AwaitingHexEncoding ) {
	    ...
	    dataNeeded = 0;
	    if ( hexEncoding & Raw ) {
		int rw = qMin(16,(int)w);
		int rh = qMin(16,(int)h);
